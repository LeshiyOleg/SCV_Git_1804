...
### 11.1. Общая информация по работе с удаленными репозиториями
...
git push
```
### 11.2. Запрос на принятие изменений (Pull Request)
Чтобы принять участие в чьем-то проекте, можно воспользоваться опцией **_Pull request_**.

Для этого сначала следует зайти на страницу проекта и нажать кнопку 

![FORK](forkbutton.png)

Таким образом создастся наша копия проекта.

Далее с помощью команды 
```
**_`git clone`_**
```
 клонируем репозиторий с проектом себе в локальное хранилище, создаем новую ветку с помощью команды 
 ```
 **_`git branch`_**
 ```
 вносим в проект наработки, сохраняем их с помощью команды 
 ```
 **_`git commit`_**
 ```
 и отправляем изменения в свою копию в удаленном репозитории командой 
 ```
 **_`git push`_**
 ```
Далее заходим на страничку нашей копии проекта (форка) на GitHub и подтверждаем открытие запроса на слияние, т.е.  запускаем процесс **_Pull request_**.
Если владельцу проекта будут актуальны и интересны наши наработки, он произведет слияние.